<template>
    <div class="content">
        ...
        <div class="d-flex justify-content-between">
            <pageheader :title="`สรุปใบเสร็จ`" title1="" />
        </div>
                    ...
                    <Form @submit="onSerch" class="flex w-full row p-0">
                        <div class="col-12 col-md-5">
                            <div class="flex justify-content-center ">
                                <label class=" font-semibold text-gray-600 text-center"
                                    for="birth_date">วันที่เริ่มต้น</label>
                            </div>
                            <Calendar id="received_date" v-model="date_s" showIcon placeholder="วันที่เริ่มต้น"
                                class="w-full" :dateFormat="customDateFormat(date_s)" showButtonBar />
                            <small class="px-2 mt-1 text-red-600">{{ errors.date_s }}</small>
                        </div>
                        <div class="col-12 col-md-5">
                            <div class="flex justify-content-center ">
                                <label class=" font-semibold text-gray-600 text-center"
                                    for="birth_date">วันที่สิ้นสุด</label>
                            </div>
                            <Calendar id="received_date" v-model="date_e" showIcon placeholder="วันที่สิ้นสุด"
                                class="w-full" :dateFormat="customDateFormat(date_e)" :minDate="date_s" showButtonBar />
                            <small class="px-2 mt-1 text-red-600">{{ errors.date_e }}</small>
                        </div>

                        <div class="col-12 col-md-2 flex align-items-center ">

                            <Button class=" h-3rem w-7rem mt-3 text-lg" :loading="loadingBtn" severity="danger"
                                icon="pi pi-search" label="ค้นหา" type="submit" />
                        </div>
                    </Form>
                </div>
            </div>
        ...
        <section class="" v-if="isShowData">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <address>
                                    ...
                                    <abbr title="Phone"><i class="pi pi-phone"></i> :</abbr> {{ resBranch.phone_number
                                    }}<br>
                ...
                <div class="col-lg-6 col-sm-6 col-12  d-flex align-items-stretch">
                    <div class="dash-widget" style="flex: 1">
                        ...
                        <div class="dash-widgetcontent">
                            <h6>ยอดขายรายวัน</h6>
                            <h5>
                                ฿<vue3-autocounter class="counter" ref="counter" :startAmount="0" decimalSeparator="."
                                    :decimals="2" :delay="3" :endAmount="resGetShiftSummaryDaily.sum_amount" :duration="5"
                                    :autoinit="true" />
                            </h5>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title">ข้อมูลสรุปใบเสร็จ</h4>
                    <div class="table-responsive">
                        <DataTable :exportFilename="`ข้อมูลสรุปใบเสร็จ${format(new Date(), 'ddMMyyyyHHmmss')}`" v-model:filters="filters" v-model:selection="selectedRowTable" class="p-datatable-sm"
    ...
    paginator: {
        ...
        pageButton: ({ props, state, context }) => ({
            class: context.active ? 'bg-orage-600 border-none border-round-lg mr-1' : 'bg-gray-600 border-none border-round-lg text-white mr-1'
                            ...
                            <Column header="จำนวนเงิน" field="total_amount" sortable="total_amount"
                                style="min-width: 12rem">
                                <template #sorticon="slotProps">
                                    <div v-if="slotProps.sortOrder === 0 || slotProps.sortOrder === 1">
                                        <i class="pi pi-angle-down" style="font-size: 12px"></i>
                            ...
                            <Column header="รับเงิน" 
                                style="min-width: 8rem">
                               
                            </Column>
                            <Column header="ทอนเงิน" 
                                style="min-width: 8rem">
...
const breadcrumbItems = [
    { label: 'สรุปใบเสร็จ', to: '/summaryofwork' },
]
