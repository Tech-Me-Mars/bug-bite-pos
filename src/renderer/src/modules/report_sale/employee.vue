<template>
  <div class="content">
    ...
    <div class="d-flex justify-content-between">
      <pageheader :title="`สรุปยอดขายโดยบุคลากร`" title1="" />
    </div>
    <div class="card mb-2">
      <div class="card-body">
        ...
        <div class="col-lg-6 col-sm-6 col-12 d-flex align-items-stretch">
          <div class="dash-widget dash1" style="flex: 1">
            ...
            <div class="dash-widgetcontent">
              <h6>จำนวนใบเสร็จ</h6>
              <h5>
                <vue3-autocounter v-if="resGetShiftSummaryDaily.count_bill" class="counter" ref="counter" :startAmount="0"
                  :delay="3" :endAmount="resGetShiftSummaryDaily.count_bill" :duration="5" :autoinit="true" />
        ...
        <div class="col-lg-4 col-sm-6 col-12 d-flex align-items-stretch">
          <div class="dash-widget" style="flex: 1">
            ...
            <div class="dash-widgetcontent">
              ...
              <h5>
                ฿<vue3-autocounter class="counter" ref="counter" :startAmount="0" decimalSeparator="." :decimals="2"
                  :delay="3" :endAmount="resGetShiftSummaryDaily.sum_amount_qr_payment" :duration="5" :autoinit="true" />
              </h5>
            </div>
          </div>
        </div>
        ...
        <div class="col-lg-4 col-sm-6 col-12 d-flex align-items-stretch">
          <div class="dash-widget" style="flex: 1">
            ...
            <div class="dash-widgetcontent">
              ...
              <h5>
                ฿<vue3-autocounter class="counter" ref="counter" :startAmount="0" decimalSeparator="." :decimals="2"
                  :delay="3" :endAmount="resGetShiftSummaryDaily.sum_amount_credit_card" :duration="5" :autoinit="true" />
              </h5>
            </div>
          </div>
        </div>
    ...
    pageButton: ({ props, state, context }) => ({
      class: context.active
        ...
        : 'bg-gray-600 border-none border-round-lg text-white mr-1'
              ...
              <Column header="สถานะการเคลม" field=""
                  ...
                  <div class="flex gap-1">
                    ...
                    <router-link :to="`/claimproduct/${data.id}`">
                      <Button icon="pi pi-replay" severity="success" class="w-full md:w-8rem" out
                        style="background-color: rgb(76, 173, 72)">
                        <i class="fa fa-wrench" data-bs-toggle="tooltip" title="fa fa-wrench"></i>เคลมสินค้า
                      </Button>
                    </router-link>
                  </div>
                </template>
  ...
  <Dialog v-model:visible="isShowDialogClaimdetail" maximizable modal header="ข้อมูลการเคลม" :style="{ width: '900px' }">
    <!-- {{ resGetClaimDetail }} -->
    <div class="p-4">
...
const loadDataSearch = async () => {
  ...
  try {
    const res = await dataApi.getSummaryReceipt(date_sFormat, date_eFormat, sku_barcode.value,user_id,payment_method_id.value)
...
const breadcrumbItems = [{ label: 'สรุปยอดขายโดยบุคลากร', to: '/report-sale-employee' }]
